import { ArrowRight, Info, Pencil, PlusCircle, Trash2, ArrowUp, ArrowDown, FileStack } from "lucide-react";
...
import ExpertiseDetailsPanel from "../../ui/Courses/details/ExpertiseDetailsPanel";
import MaterialManager from "../../ui/Courses/Material";

interface EspertiseQueryResponse extends Response {
  data: {
    id: string;
    name: string;
    description: string;
...
type Expertise = EspertiseQueryResponse["data"][0];

export default function ViewExpertise() {
  ...
  const [selectedExpertiseForDetails, setSelectedExpertiseForDetails] =
  ...
  const [editForm, setEditForm] = useState<{
    ...
    isPublished: boolean;
    levels: string[];
  }>({ name: "", description: "", tumbnailUrl: "", isPublished: true, levels: [] });
  const [materialManagerOpen, setMaterialManagerOpen] = useState(false);
  const [selectedForMaterials, setSelectedForMaterials] = useState<Expertise | null>(null);
  ...
  const reorderMutation = useMutation({
    mutationFn: async (payload: { id: string; direction: "up" | "down" }) => {
      ...
      const res = await api.put(
        API_ROUTES.COURSE.UPDATE.REORDER_EXPERTISE(payload.id),
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ["expertise", skillCategoryId],
      });
  ...
  const updateMutation = useMutation({
    mutationFn: async (payload: { id: string; data: any }) => {
      const res = await api.put(
        API_ROUTES.COURSE.UPDATE.EXPERTISE(payload.id),
        payload.data
      );
      return res.data;
    },
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const res = await api.delete(API_ROUTES.COURSE.DELETE.EXPERTISE(id));
      return res.data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({
  ...
  const skillCategoryQuery = useQuery({
    ...
    queryFn: async () => {
      const res = await api.get(
        API_ROUTES.COURSE.GET_BY_ID.SKILL_CATEGORY(skillCategoryId!)
      );
  ...
  const openDelete = (row: Expertise) => {
    setSelected(row);
    setDeleteOpen(true);
  };
  ...
  const getExpertiseQuery = useQuery({
    queryKey: ["expertise", skillCategoryId],
    queryFn: async () => {
      const res = await api.get<EspertiseQueryResponse>(
        API_ROUTES.EXPERTISE.GET_EXPERTISE_BY_SKILL_CATEGORY_ID(
          skillCategoryId!
        )
      );
      return res.data;
  ...
  const data = useMemo(
    () => getExpertiseQuery.data?.data || [],
    [getExpertiseQuery]
  );

  const columns = useMemo<ColumnDef<Expertise>[]>(
    () => [
      {
        header: "#",
        ...
        cell: ({ row, table }) => {
          ...
          return (
            <div className="flex items-center gap-1.5 flex-wrap">
              <button
                className={`p-1.5 rounded-lg transition-all duration-300 ${canMoveUp && !isReordering
              ...
              <button
                className={`p-1.5 rounded-lg transition-all duration-300 ${canMoveDown && !isReordering
                    ? "text-[var(--primary)] hover:bg-[var(--primary)]/10 hover:scale-110 active:scale-95"
                    : "text-[var(--muted-foreground)] opacity-50 cursor-not-allowed"
                  }`}
                onClick={() => {
              ...
              <button
                onClick={() => {
                  setSelectedExpertiseForDetails(row.original);
              ...
              onClick={() => {
                const modulePath =
                  isContributorRoute || isContributor
                    ? `/contributor/module/${row.original.id}`
                    : `/admin/module/${row.original.id}`;
                router.push(modulePath, "Expertise");
              }}
  ...
  return (
    <div className="bg-[var(--background)] min-h-screen transition-colors duration-300">
      <TopBar>
        <button
          ...
          onClick={() => {
            const createPath =
              isContributorRoute || isContributor
                ? `/contributor/expertise/create/${skillCategoryId}`
                : `/admin/expertise/create/${skillCategoryId}`;
            router.push(createPath, "Create Domain");
          }}
        >
      ...
      <div className="mt-8 px-8 pb-8">
        ...
        <div className="mt-4 rounded-xl border border-[var(--border)] bg-[var(--card)] overflow-hidden shadow-sm">
          <TanstackTable
            ...
            isLoading={getExpertiseQuery.isLoading}
            isError={getExpertiseQuery.isError}
            isEmpty={
              !getExpertiseQuery.isLoading &&
              getExpertiseQuery.data?.data.length === 0
            }
          />
        </div>
      </div>
          ...
          <div className="p-6 flex flex-col gap-4 overflow-y-auto flex-1">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-4">
                {(['LEARNER', 'PRACTITIONER', 'PROFESSIONAL']).map((level) => (
                  <label
                    key={level}
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        setEditForm((prev) => {
                          ...
                          if (e.target.checked) {
                            return { ...prev, levels: [...current, level] };
                          } else {
                            return { ...prev, levels: current.filter((l: string) => l !== level) };
                          }
                        });
                      }}
      ...
      />
      {detailsPanelOpen && selectedExpertiseForDetails && (
        <ExpertiseDetailsPanel
          isOpen={detailsPanelOpen}
          onClose={() => {
            setDetailsPanelOpen(false);
            setSelectedExpertiseForDetails(null);
          }}
          expertiseId={selectedExpertiseForDetails.id}
          courseId={courseId}
        />
      )}
